...
Page({
  ...
  data: {
    ...
    tabs: ["赛事", "活动"],
    tabsValue: ["COMPETITION", "EXPERIENCE"],
  ...
  onLoad: function (options) {
    ...
    that.setData({
      ...
      loginCalled: app.globalData.loginCalled
  ...
  loadData: function () {
    ...
    utils.sendRequest({
      url: '/api/competition/resultlist',
      data: {
      ...
      for (var i = 0; i < res.list.length; i++) {
        ...
        res.list[i].joinTime = utils.formatTime(new Date(res.list[i].joinTime));
  ...
  clickCancel: function (e) {
    ...
    utils.sendRequest({
      url: '/api/signorder/cancelsignorder',
      data: {
        acSignOrderUuid: e.currentTarget.dataset.uuid,
  ...
  clickTopay: function (e) {
    ...
    utils.pay_request(e.currentTarget.dataset.uuid, "redirect", e.currentTarget.dataset.activityId);
