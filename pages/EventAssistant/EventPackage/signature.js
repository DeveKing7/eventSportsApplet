  ...
  showCanvas: function () {
    ...
    wx.canvasToTempFilePath({
      ...
      success: function (res) {
        ...
        var canvas = wx.createContext("wlt");
        // that.ctx.translate(0, that.data.canvasHeight/2.0);
        // that.ctx.rotate(-90 * Math.PI / 180);
        // that.ctx.scale(0.5, 0.5)
        // that.ctx.drawImage(res.tempFilePath, 0, 0, that.data.canvasWidth, that.data.canvasHeight, that.data.canvasWidth, 0, that.data.canvasWidth, that.data.canvasHeight);
       
        // that.ctx.draw();
      
        // canvas.drawImage(res.tempFilePath, 0, 0, that.data.canvasWidth,that.data.canvasHeight );
        // wx.drawCanvas({
          ...
          wx.uploadFile({
            ...
            success: function (res) {
              ...
              if (!resJson.hasErrors) {
                ...
                utils.sendRequest({
                  url: '/api/function/setsignature',
                  data: {
                    fileUrl: resJson.items[0].url,
